...
in
lib.mkMerge [
  (lib.mkIf config.mo.programs.ssh.enableKey {
    home.activation.ensureSSH = lib.hm.dag.entryAfter [ "sops-nix" ] ''
      set -euo pipefail
      mkdir -p "${sshDir}"
      chmod 700 "${sshDir}"
  ...
  (lib.mkIf config.mo.programs.ssh.enableHost {
    ...
    home.activation.writeSshConfig = lib.hm.dag.entryAfter [ "sops-nix" ] ''
      ...
      install -m 644 "${config.sops.templates."ssh-config.conf".path}" "${sshDir}/config"
    '';
  })
]
